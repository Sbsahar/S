...
# بالنسخة الجديدة:
group_detection_status = load_group_status()
reports_data = load_reports_status()
activated_groups = reports_data.get("activated_groups", {})
daily_reports = reports_data.get("daily_reports", {})
REPLIES_FILE = "replies.json"
# إعداد ملفات الحفظ
GROUP_STATUS_FILE = "group_status.json"
REPORTS_FILE = "reports_status.json"
...
def smart_detector(message):
    ...
    welcome_msg = (
        ...
        "• لفحص الفيديو والمتحركات أضف البوت المساعد  \n"
...
def handle_detector_callback(call):
    ...
    if 'toggle' in call.data:
        ...
        save_group_status()  # <-- حفظ التغييرات في الملف
                ...
                for link in sticker_links:
                    try:
                        ...
                        if res == 'nude':
                            bot.delete_message(message.chat.id, message.message_id)
                            warning_message = (
                                f"🚫 <b>لا تبعت ملصقات مميزة غير لائقة يا {message.from_user.first_name}!</b>\n"
                                f"🥷🏻 @{message.from_user.username or str(message.from_user.id)}، <b>هذا تنبيه لك!</b>\n"
                                "<b>🤖 البوت يراقب ويمنع المحتوى غير الملائم 🛂</b>"
                            )
                            bot.send_message(message.chat.id, warning_message, parse_mode="HTML")
